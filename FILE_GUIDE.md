...
### 목적
CMOS Inverter를 구성하려면 NMOS뿐 아니라 PMOS의 Proposed 구조도 필요하다.

### 확인할 항목
- NMOS와 같은 device metric
- PMOS drive current
- NMOS/PMOS delay balance
- Inverter switching point 변화
...
공정 및 소자 변화가 실제 회로 Power와 Delay에 미치는 영향을 검증한다.

## `04_circuit/inverter/inverter.cmd.template`

### 역할
NMOS와 PMOS를 CMOS Inverter로 연결하는 MixedMode 또는 회로 Deck 초안이다.
...
### 언제 수정하는가
Baseline과 Proposed NMOS/PMOS가 모두 수렴한 뒤 수정한다.
...
### 역할
3-stage 또는 5-stage Ring Oscillator를 구성하는 회로 Deck 초안이다.
